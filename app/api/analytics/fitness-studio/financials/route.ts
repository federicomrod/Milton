// GET /api/analytics/fitness-studio/financials?from_date=...&to_date=...&chart=income-statement|variance-analysis|ytd-performance
// Returns financial chart data (income statement, variance analysis, YTD performance)
...
import type { TransactionData, BudgetData } from "@/lib/types/data";
...
export async function GET(req: NextRequest) {
  try {
    ...
    const chartType =
      req.nextUrl.searchParams.get("chart") || "income-statement";
    const fromDate =
    ...
    while (hasMore) {
      ...
      if (result.error) {
        console.error("[financials] Error fetching model_data:", result.error);
        break;
      }
    ...
    const budgetsData = allModelData.filter(
      (row) =>
        idToNameMap[row.model_table_id]?.includes("budget") ||
        idToNameMap[row.model_table_id]?.includes("plan")
    );
    ...
    // Normalize budgets
    const normalizeBudget = (b: any): BudgetData => {
      return {
    ...
    // Normalize all transactions and budgets
    ...
    console.error(
      "[api/analytics/fitness-studio/financials] Unexpected error:",
      err
    );
    return jsonNoStore({ data: [] });
  }
