...
export default function MiltonChat({
  ...
  function nextStep() {
    if (step === "intro") {
      ...
      setMessages((prev) => [
        ...prev,
        { from: "milton", text: "Great! What industry are you in?" },
      ]);
    } else if (step === "industry") {
      setStep("employees");
      setMessages((prev) => [
        ...prev,
        { from: "milton", text: "Got it. How many employees do you have?" },
      ]);
  ...
  return (
    <div className="p-6 space-y-4">
      {messages.map((m, i) => (
        <div
          key={i}
          className={`flex ${m.from === "milton" ? "justify-start" : "justify-end"}`}
        >
          <Card
