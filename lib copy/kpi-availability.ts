...
function modelHasTable(model: ModelProposal | null | undefined, tableId: string): boolean {
  if (!model) return false
  return model.recommendedTables?.some(
...
export function evaluateKpiAvailability(
  ...
  return templates.map((tpl) => {
    ...
    if (businessType === 'fitness_studio') {
      if (tpl.id === 'class_utilization' || tpl.id === 'avg_attendance_per_class') {
        const ok = modelHasTable(model, 'bookings') && modelHasTable(model, 'classes')
        return ok
      ...
      if (tpl.id === 'cancellation_rate') {
        ...
        return ok
          ...
          : {
              id: tpl.id,
              status: 'requiresData',
              reason: 'Requires a bookings table with status/cancellation info.',
            }
      }

      if (tpl.id === 'instructor_margin') {
        const ok = modelHasTable(model, 'bookings') && modelHasTable(model, 'instructors')
        return ok
          ? { id: tpl.id, status: 'available' }
          : {
              id: tpl.id,
              status: 'requiresData',
              reason: 'Requires instructors and bookings linked to them plus their cost.',
            }
      }
