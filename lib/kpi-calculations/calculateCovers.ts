...
export async function calculateCovers(
  ...
  const monthlyCovers: { [key: string]: number } = {};
  ordersInRange.forEach((o) => {
    const orderDate = parseDate(o.date);
    if (!orderDate) return;
    const period = orderDate.toISOString().substring(0, 7); // YYYY-MM
    monthlyCovers[period] = (monthlyCovers[period] || 0) + o.covers;
  });

  const historicalData = Object.entries(monthlyCovers)
    .map(([period, covers]) => ({
      period,
      value: Math.round(covers),
    }))
    .sort((a, b) => a.period.localeCompare(b.period));

  return {
    currentValue: Math.round(totalCovers),
    historicalData,
  };
}
