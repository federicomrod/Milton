  ...
  try {
      ...
      : [
          {
            role: "system",
            content:
              "You are an expert financial data analyst specializing in German business data. Always respond with valid JSON only, no additional text or formatting.",
          },
          {
            role: "user",
            content: prompt,
    ...
    try {
      ...
      return NextResponse.json({
        ...
        confidence: 0.7,
        reasoning:
          "Fallback analysis: Could not parse AI response but detected financial data patterns",
        columnMappings: [],
        categories: [],
        businessInsights: {
          revenueColumns: [],
    ...
    const validatedResponse = {
      ...
      businessInsights: {
        ...
        recurringRevenue:
          parsedResponse.businessInsights?.recurringRevenue || [],
        dateFormat: parsedResponse.businessInsights?.dateFormat || "unknown",
        numberFormat:
          parsedResponse.businessInsights?.numberFormat || "unknown",
        primaryAmountColumn:
          parsedResponse.businessInsights?.primaryAmountColumn || "",
      },
    };
