...
export async function GET(req: NextRequest) {
  try {
    ...
    const [
      ...
      calculateNetCashFlow(supabase, user.id, fromDate, toDate).catch(() => ({
        currentValue: 0,
        historicalData: [] as { period: string; value: number }[],
      })),
      calculateTotalClassesHeld(supabase, user.id, fromDate, toDate).catch(
        () => ({ currentValue: 0, historicalData: [] })
      ),
    ]);

    // Total revenue: prefer orders/invoices (layer); fallback to transactions-derived for fitness studios with only transaction data
    ...
    const netCashFlow = netCashFlowRes.currentValue ?? 0;
    const totalClassesHeld = totalClassesHeldRes.currentValue ?? 0;
    ...
    return jsonNoStore({
      kpis: {
        ...
        netCashFlow: parseFloat(Number(netCashFlow).toFixed(2)),
