    ...
    if (data.transactions && Array.isArray(data.transactions)) {
      ...
      analysis.transactions = {
        ...
        amountDistribution: {
          ...
          avg:
            txs.reduce((sum: number, tx: any) => sum + tx.amount, 0) /
            txs.length,
        },
      };
    ...
    <div className="space-y-6">
      <Card className="border-yellow-200 bg-yellow-50">
        <CardHeader>
          ...
          <CardDescription>
            Debug what's actually stored in localStorage and identify processing
            issues
          </CardDescription>
        </CardHeader>
        <CardContent>
                ...
                <Card>
                  ...
                  <CardContent>
                    ...
                    <div className="text-xs text-gray-600">
                      Total: €
                      {(
                        analysis.transactions?.totalAmount || 0
                      ).toLocaleString()}
                    </div>
                    <div className="text-xs text-gray-600">
                  ...
                  <>
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      ...
                      <div>
                        ...
                        {analysis.transactions.totalAmount?.toLocaleString()}
                      </div>
                      <div>
                        <strong>Positive/Negative:</strong>{" "}
                        {analysis.transactions.positiveAmount}/
                        {analysis.transactions.negativeAmount}
                      </div>
                      <div>
                        <strong>Recurring Revenue Txs:</strong>{" "}
                        {analysis.transactions.recurringRevenue}
                      </div>
                      <div>
                        <strong>Date Range:</strong>{" "}
                        {analysis.transactions.dateRange?.earliest} to{" "}
                        {analysis.transactions.dateRange?.latest}
                      </div>
                      <div>
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1 mt-1">
                        {analysis.transactions.categories?.map(
                  ...
                  <>
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      ...
                      <div>
                        ...
                        {analysis.crm.totalValue?.toLocaleString()}
                      </div>
                      <div>
                        <strong>Unique Clients:</strong>{" "}
                        {analysis.crm.clients?.length}
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1 mt-1">
                        {analysis.crm.phases?.map(
                          (phase: string, idx: number) => (
                            <Badge
                              key={idx}
                              variant="outline"
                              className="text-xs"
                ...
                {analysis.budget ? (
                  <>
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div>
                        <strong>Categories:</strong>{" "}
                        {analysis.budget.categories?.length}
                      </div>
                      <div>
                        <strong>Months:</strong>{" "}
                        {analysis.budget.months?.length}
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1 mt-1">
                        {analysis.budget.categories?.map(
                          (cat: string, idx: number) => (
                            <Badge
                              key={idx}
                              variant="outline"
