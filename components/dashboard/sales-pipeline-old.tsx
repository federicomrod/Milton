  ...
  const calculatePipelineMetrics = (dealsData: Deal[]) => {
    ...
    const phaseData = phaseOrder.map((phase) => {
      ...
      return {
        ...
        avgValue:
          phaseDeals.length > 0
            ? phaseDeals.reduce((sum, d) => sum + d.amount, 0) /
              phaseDeals.length
            : 0,
      };
    });
    ...
    const funnelData = activePhasesData.map((phase, index) => {
      ...
      return {
        ...
        value: value,
        percentage:
          dealsData.filter((d) => d.phase !== "No Deal").length > 0
            ? (
                (cumulativeCount /
                  dealsData.filter((d) => d.phase !== "No Deal").length) *
                100
              ).toFixed(1)
            : 0,
        ...
        dealsData.forEach((deal) => {
          if (deal.closingDate) {
            ...
            if (
              ...
              monthData[deal.phase] =
                (monthData[deal.phase] || 0) + deal.amount;
              monthData[`${deal.phase}_weighted`] =
                (monthData[`${deal.phase}_weighted`] || 0) +
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {
                deals.filter((d) => d.phase !== "No Deal" && d.phase !== "Deal")
                  .length
              }
            </div>
          </CardContent>
                    ...
                    return (
                      <div className="bg-white p-4 border rounded shadow-lg">
                        ...
                        <p className="text-sm font-semibold mt-2 pt-2 border-t">
                          Total: €
                          {payload[0].payload[totalKey].toLocaleString()}
                        </p>
                      </div>
                    );
                  }
              ...
              <PieChart>
                ...
                >
                  {metrics.dealsByProduct.map((entry: any, index: number) => (
                    <Cell
                      key={`cell-${index}`}
                      fill={
                        Object.values(currentColors)[
                          index % Object.values(currentColors).length
                        ]
                      }
                    />
                  ))}
                ...
                <Legend
                  ...
                  formatter={(value: any, entry: any) =>
                    `${entry.payload.product}: €${(entry.payload.value / 1000).toFixed(0)}k`
                  }
                />
              </PieChart>
            </ResponsiveContainer>
