...
interface DataMappingUIProps {
  ...
  onCancel: () => void;
  onReanalyze?: () => void;
  /** Called when column mappings change (for multi-file flow to preserve draft when switching file). */
  onDraftChange?: (mappings: ColumnMapping[]) => void;
  ...
  /** When true, render inline (no Dialog) so parent can show a sidebar (e.g. multi-file mapping). */
...
export default function EnhancedDataMappingUI({
  ...
  // Notify parent of draft mappings when they change (for multi-file sidebar). Compare by content to avoid loops.
  const lastDraftRef = useRef<string>("");
  useEffect(() => {
    if (!onDraftChange) return;
    const key = JSON.stringify(effectiveMappings);
    if (key === lastDraftRef.current) return;
    lastDraftRef.current = key;
    onDraftChange(effectiveMappings);
  }, [effectiveMappings, onDraftChange]);
  ...
  const runAiMapping = useCallback(async () => {
    const targetFields =
      isModelTableMode && modelTableFields?.length
        ? modelTableFields.map((f) => ({
            name: f.name,
            type: f.type,
            required: f.required,
          }))
    ...
    try {
      const res = await fetch("/api/ai/column-mapper", {
        ...
        body: JSON.stringify({
          headers,
          sampleRows: (allRows && allRows.length > 0
            ? allRows
            : sampleData
          ).slice(0, 5),
          targetFields,
          tableName: modelTableName ?? fileType,
        }),
      });
      if (!res.ok) throw new Error("AI mapping request failed");
      const data = await res.json();
      const suggested = Array.isArray(data.mappings) ? data.mappings : [];
      if (isModelTableMode && modelTableFields?.length) {
        setFieldToColumn((prev) => {
          const next: Record<string, string> = { ...prev };
          for (const f of modelTableFields) {
            const m = suggested.find(
              (s: ColumnMapping) => s.standardField === f.name
            );
            const aiColumn =
              m?.originalColumn && headers.includes(m.originalColumn)
                ? m.originalColumn
                : null;
            if (aiColumn) {
              next[f.name] = aiColumn;
            }
            // Keep prev[f.name] when AI didn't return a valid mapping
          }
          return next;
        });
      } else {
        setMappings((prev) => {
          const converted: ColumnMapping[] = headers.map((header) => {
            const m = suggested.find(
            ...
            const aiTarget =
              m?.standardField && m.standardField !== "unmapped"
                ? m.standardField
                : null;
            const existing = prev.find((p) => p.originalColumn === header);
            const sampleValue = sampleData[0]?.[header];
            let dataType: "string" | "number" | "date" | "currency" = "string";
            if (sampleValue !== null && sampleValue !== undefined) {
              if (typeof sampleValue === "number") dataType = "number";
            ...
            return {
              originalColumn: header,
              standardField: aiTarget ?? existing?.standardField ?? "unmapped",
              confidence: aiTarget
                ? (m?.confidence ?? 0)
              ...
              dataType:
                ...
                dataType,
              transformation:
                (m?.transformation as ColumnMapping["transformation"]) ??
      ...
      console.error("[data-mapping] AI mapping error:", err);
  ...
  const headerContent = (
    <div className="flex-shrink-0 flex items-center gap-2">
      ...
      <span className="text-lg font-semibold">Data Mapping - {fileName}</span>
  ...
  const innerContent = (
    <div className="flex flex-col h-full overflow-hidden px-6 py-4">
      ...
      <div className="flex flex-col flex-1 min-h-0">
        {/* Status Bar – always visible (single and multi-file upload); includes AI Mapping */}
        <div className="flex items-center justify-between p-4 pr-6 bg-gray-50 dark:!bg-gray-800 rounded-lg mb-4 flex-shrink-0 sticky top-0 z-10">
          ...
          <Alert className="mb-4 flex-shrink-0 border-amber-300 dark:border-amber-700 bg-amber-50 dark:bg-amber-950/30">
            ...
            <AlertDescription>
              <p className="text-sm font-medium text-amber-800 dark:text-amber-200">
                ...
                {missingDependencies.length === 1 ? "has" : "have"} no data yet
              </p>
              <p className="text-sm text-amber-700 dark:text-amber-300 mt-1">
                ...
                {missingDependencies.length === 1
                  ? "this table"
                  : "these tables"}{" "}
                won&apos;t be linked automatically. You can still upload now and
                the references will be resolved when the dependent data is
                available.
              </p>
            </AlertDescription>
          </Alert>
            ...
            {isModelTableMode ? (
              <div className="space-y-2">
                ...
                <div className="grid gap-3">
                  {[...standardFields]
                    .sort((a, b) => {
                      const aPk = "isPrimaryKey" in a && a.isPrimaryKey ? 1 : 0;
                      const bPk = "isPrimaryKey" in b && b.isPrimaryKey ? 1 : 0;
                ...
                {(fieldsWithAllowedValues.length > 0 || isNormalizing) && (
                  <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-400 mb-3">
                      Fields with constrained values are listed below. Accept
                      the AI suggestion, pick a different canonical value, or
                      choose &quot;Keep original&quot; to leave the value
                      unchanged.
                    </p>
                    {fieldsWithAllowedValues.map((field) => {
                      const mapping = valueMappings[field.value] ?? {};
                      const allVals = uniqueValuesPerField[field.value] ?? [];
                      if (allVals.length === 0) return null;
                      const changedCount = allVals.filter(
                        (v) => mapping[v] && mapping[v] !== v
                      ).length;
                      const keptCount = allVals.filter(
                        (v) => mapping[v] === "__keep__"
                      ).length;
                      return (
                        <Card
                          key={field.value}
                          ...
                          <CardContent className="p-4">
                            <div className="flex items-center gap-2 mb-3">
                              ...
                              {keptCount > 0 && (
                                ...
                                >
                                  {keptCount} kept original
                                </Badge>
                              )}
                            </div>
                            <div className="space-y-2">
                              {allVals.map((original) => {
                                const canonical = mapping[original];
                                const isKeep = canonical === "__keep__";
                                const isChanged =
                                  canonical &&
                                  canonical !== original &&
                                  !isKeep;
                                ...
                                return (
                                  ...
                                  >
                                    <span className="font-mono text-sm text-gray-600 dark:text-gray-400 w-40 truncate flex-shrink-0">
                                      {original}
                                    </span>
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  <div>
                    <span className="text-gray-600 dark:text-gray-400">
                      {isModelTableMode ? "Mapped fields:" : "Mapped Columns:"}
                    </span>
        ...
        <div className="flex justify-between items-center pt-4 border-t border-gray-200 dark:border-gray-700 flex-shrink-0 mt-4">
          ...
          <div className="flex gap-2">
            ...
            <Button
              onClick={() => {
                // Resolve __keep__ sentinels: replace with the original value
                const resolvedMappings: Record<
                  string,
                  Record<string, string>
                > = {};
                for (const [fieldName, fm] of Object.entries(valueMappings)) {
                  const resolved: Record<string, string> = {};
                  for (const [orig, canonical] of Object.entries(fm)) {
                    resolved[orig] =
                      canonical === "__keep__" ? orig : canonical;
                  }
                  resolvedMappings[fieldName] = resolved;
                }
                onConfirm(
                  effectiveMappings,
                  Object.keys(resolvedMappings).length > 0
                    ? resolvedMappings
                    : undefined
                );
              }}
            ...
            >
              ...
              Apply Mapping (
              {
                effectiveMappings.filter((m) => m.standardField !== "unmapped")
                  .length
              }{" "}
              fields)
            </Button>
  ...
  if (embedInLayout) {
    return (
      <div className="flex flex-col h-full min-h-0 overflow-hidden rounded-xl border bg-background">
        {innerContent}
      </div>
    );
  }
  return (
    <Dialog open={true} onOpenChange={() => onCancel()}>
      <DialogContent
        className="!max-w-[calc(100vw-2rem)] w-[calc(100vw-2rem)] h-[calc(100vh-2rem)] max-h-[calc(100vh-2rem)] flex flex-col p-0 rounded-xl overflow-hidden"
