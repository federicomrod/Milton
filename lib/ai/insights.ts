...
export async function generateInsights(
  ...
  try {
    ...
    switch (section) {
      case "overview":
        return [
          `Revenue ${fmt(k.revenue)}; expenses ${fmt(k.expenses)}; net income ${fmt(k.netIncome)}.`,
          defaultLine,
        ];
      case "financial":
      ...
      case "pipeline":
        return [
          `Pipeline value ${fmt(k.pipelineValue)} across ${k.openDeals ?? 0} open deals.`,
          defaultLine,
        ];
      case "cashflow":
        return [
          `Burn rate ${fmt(k.burnRate)} per month; runway ${k.cashRunway ?? 0} months.`,
          defaultLine,
        ];
    }
  }
