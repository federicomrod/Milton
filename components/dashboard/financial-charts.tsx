...
export function FinancialCharts({ type }: ChartProps) {
  ...
  const generateMRRChart = (
    ...
    const actualMRR =
      recurringRevenue > 0 ? recurringRevenue : totalMonthlyRevenue;

    // Get planned MRR from budget for the current month being analyzed
    const budgetMonth = targetMonth.toLocaleString("default", {
      month: "short",
      year: "numeric",
    });
  ...
  const generateIncomeStatement = (transactions: any[], budget: any) => {
    ...
    const waterfallData = [{ name: "Revenue", value: totalRevenue }];
  ...
  const generateYTDPerformance = (transactions: any[], budget: any) => {
    ...
    for (let month = 0; month <= new Date().getMonth(); month++) {
      ...
      ytdData.push({
        ...
        cumBudget:
          ytdData.reduce((sum, d) => sum + d.budget, 0) + plannedRevenue,
      });
    }
