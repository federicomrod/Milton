...
export async function upsertUserProfile(profileData: Partial<UserProfile>): Promise<UserProfile> {
  ...
  const { email, company_name, industry, ...cleanProfileData } = profileData as any
...
export async function updateCompany(companyData: Partial<Company>): Promise<Company> {
  const supabase = createClient()
  const { data: { user }, error: userError } = await supabase.auth.getUser()
  ...
  if (companyError) {
    console.error('Company update error:', companyError.message, companyError.details, companyError.hint)
    throw companyError
  }
